...
def input_params(
    ...
    if tasks.high_modes:
        ...
        w_indices = (
            tasks.w_indices if tasks.w_indices else list(range(submit_config.noise))
        )
        v_indices = (
            tasks.v_indices if tasks.v_indices else list(range(submit_config.noise))
        )
        pairings = list(
            sorted(
                filter(
                    lambda x: x[0] != x[1],
                    set((tuple(x) for x in itertools.product(w_indices, v_indices))),
                )
            )
        )
        module_set = set()
